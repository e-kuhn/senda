...
def generate_domain_builder(schema: ExportSchema) -> str:
        ...
        for mi, m in enumerate(c.members):
            if m.xml_element_name:
                ...
                if ri is not None:
                    ...
                    xml_tags = 0
                    if m.xml_role_element:
                        xml_tags |= 0x01
                    if m.xml_role_wrapper_element:
                        xml_tags |= 0x02
                    if m.xml_type_element:
                        xml_tags |= 0x04
                    if m.xml_type_wrapper_element:
                        xml_tags |= 0x08
                    if m.xml_attribute:
                        xml_tags |= 0x10
    ...
    w("    fir::Fir type_fir;")
    w("    fir::FirBuilder b(type_fir);")
    w("")
    w("    constexpr size_t N = std::size(kTypes);")
    w("    std::vector<fir::TypeHandle> th(N);")
    w("    for (size_t i = 0; i < N; ++i)")
    w("        th[i] = b.add_type(kTypes[i].name, static_cast<fir::M3Kind>(kTypes[i].kind));")
    w("")
    w("    for (size_t i = 0; i < N; ++i) {")
    ...
    w("")
    w("    std::vector<fir::RoleHandle> rh(std::size(kRoles));")
    w("    for (size_t i = 0; i < N; ++i) {")
    ...
    w("                RoleInfo{rh[tr.role_index],")
    w("                         static_cast<uint32_t>(th[tr.target_type]),")
    ...
    w("    for (const auto& tag : kTags)")
    w("        handle_to_type.add(static_cast<uint32_t>(th[tag.type_index]),")
